...
class VoxelPostprocessor(BasePostprocessor):
    def __init__(self, anchor_params, train):
        ...
        self.dkf_feature_size = self.params['dkf_feature_size']
    ...
    def get_object_features(self, spatial_features_2d, spatial_features, mask):
        feature_size = self.dkf_feature_size # 5
        half_feature_size = feature_size // 2 # 2

        # The following print format: # called_by_no_fusion_keep_all, called_by_cobevt
        #print('spatial_features_2d.shape: ', spatial_features_2d.shape)
        # [1, 256, 50, 88], [1, 256, 48, 128]
        #print('spatial_features.shape: ', spatial_features.shape)
        # [1, 256 / 4, 50 * 4, 88 * 4], [1, 256 / 1, 48 * 1, 128]

        scale_factor = spatial_features_2d.shape[1] // spatial_features.shape[1]
        #print('scale_factor: ', scale_factor)
        # called_by_no_fusion_keep_all: 4, called_by_cobevt: 1

        _, feature_dim, H, W = spatial_features_2d.shape

        #print('mask.shape: ', mask.shape)
        # [1, 8800], 8800 = 50 * 88 * 2, 2 = self.anchor_num

        indices = torch.nonzero(mask[0])[:, 0]
        #print('indices.shape: ', indices.shape) # [10]
        #print('indices: ', indices) # values 0 ~ 8799
        num_objects = indices.shape[0]

        indices = torch.remainder(indices, H * W)
        #print('indices.shape: ', indices.shape) # [10]
        #print('indices: ', indices) # values 0 ~ 4399

        h_indices = torch.div(indices, W, rounding_mode='floor')
        #print('h_indices: ', h_indices) # 0 ~ 49
        w_indices = torch.remainder(indices, W)
        #print('w_indices: ', w_indices) # 0 ~ 87

        h_indices = torch.clamp(h_indices, 
          min = half_feature_size, max = H - 1 - half_feature_size)
        #print('h_indices: ', h_indices) # 0 ~ 49
        w_indices = torch.clamp(w_indices, 
          min = half_feature_size, max = W - 1 - half_feature_size)
        #print('w_indices: ', w_indices) # 0 ~ 87

        object_features = []
        for i in range(num_objects):
          object_features.append(spatial_features_2d[0, :, 
            h_indices[i] - half_feature_size : h_indices[i] + half_feature_size + 1,
            w_indices[i] - half_feature_size : w_indices[i] + half_feature_size + 1,
          ])
 
        object_features = torch.stack(object_features, dim=0)
        #print('object_features.shape: ', object_features.shape)
        # called_by_no_fusion_keep_all: [10, 256, 5, 5]: [num_objects, feature_dim, feature_size, feature_size]
        # called_by_cobevt: [4, 256, 5, 5]


        # object_early_features from spatial_features
        # 4 times larger in each spatial dimensions
        h_indices *= scale_factor
        w_indices *= scale_factor
        object_early_features = []
        for i in range(num_objects):
          object_early_features.append(spatial_features[0, :, 
            h_indices[i] - half_feature_size * scale_factor : h_indices[i] + (half_feature_size + 1) * scale_factor,
            w_indices[i] - half_feature_size * scale_factor : w_indices[i] + (half_feature_size + 1) * scale_factor,
          ])
        object_early_features = torch.stack(object_early_features, dim=0)
        #print('object_early_features.shape: ', object_early_features.shape)
        # [10, 64, 20, 20], [4, 256, 5, 5]
        

        return object_features, object_early_features


    def post_process_return_in_dict(self, data_dict, output_dict):
        """
        Process the outputs of the model to 2D/3D bounding box.
        Step1: convert each cav's output to bounding box format
        Step2: project the bounding boxes to ego space.
        Step3: NMS in each individual cav's detection result.
        Step4: return each cav's detetcion result separately.
        ...
        gt_box3d_tensor : torch.Tensor
            The groundtruth bounding box tensor.
        """
        # the final bounding box dict: cav_id to list of box
        pred_box_dict = {}
        pred_score_dict = {}
        pred_feature_dict = {}
        pred_early_feature_dict = {}

        #print('data_dict.keys(): ', data_dict.keys())
        #print('output_dict.keys(): ', output_dict.keys())
        # called from cobvt: ['ego']
        # called from no_fusion_keep_all: ['ego', '1']
        ...
        for cav_id, cav_content in data_dict.items():
            ...
            # called_by_no_fusion_keep_all, called_by_cobevt
            #print('mask.shape: ', mask.shape) # [1, 8800],  [1, 12288]
            #print('mask_reg.shape: ', mask_reg.shape) # [1, 8800, 7], [1, 12288, 7]
            #print('batch_box3d.shape: ', batch_box3d.shape) # [1, 8800, 7],  [1, 12288, 7]
            #print('boxes3d.shape: ', boxes3d.shape) # [10, 7], [4, 7]
            #print('prob.shape: ', prob.shape) # [1, 8800], [1, 12288]]
            #print('scores.shape: ', scores.shape ) # [10], [4]

            object_features, object_early_features = self.get_object_features(
              output_dict[cav_id]['spatial_features_2d'],
              output_dict[cav_id]['spatial_features'],
              mask)
            # called_by_no_fusion_keep_all, called_by_cobevt
            #print('object_features.shape: ', object_features.shape)
            # [10, 256, 5, 5], [4, 256, 5, 5]
            #print('object_early_features.shape: ', object_early_features.shape)
            # [10, 64, 20, 20], [4, 256, 5, 5]
            ...
            # For the no_fusion_keep_all setting, 
            # perform nms in each cav's detection result

            if len(pred_box2d_list) ==0 or len(pred_box3d_list) == 0:
                #return None, None
                # if this cav does not detect any object, 
                # continue to process next cav
                continue
            ...
            # select cooresponding score
            scores = scores[keep_index]
            # select corresponding object features
            object_features = object_features[keep_index]
            object_early_features = object_early_features[keep_index]
            ...
            pred_box3d_tensor = pred_box3d_tensor[mask, :, :]
            scores = scores[mask]
            object_features = object_features[mask]
            object_early_features = object_early_features[mask]

            assert scores.shape[0] == pred_box3d_tensor.shape[0]
            assert scores.shape[0] == object_features.shape[0]
            assert scores.shape[0] == object_early_features.shape[0]

            # save the final detection result
            pred_box_dict[cav_id] = pred_box3d_tensor
            pred_score_dict[cav_id] = scores 
            pred_feature_dict[cav_id] = object_features
            pred_early_feature_dict[cav_id] = object_early_features


        #print('pred_box_dict: ', pred_box_dict)
        #print('pred_score_dict: ', pred_score_dict)
        #print('pred_feature_dict: ', pred_feature_dict)
        #print('pred_early_feature_dict: ', pred_early_feature_dict)

        return pred_box_dict, pred_score_dict, pred_feature_dict, pred_early_feature_dict
