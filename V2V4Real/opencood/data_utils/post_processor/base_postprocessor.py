...
class BasePostprocessor(object):
    ...
    def generate_gt_bbx(self, data_dict):
        ...
        #print('gt_box3d_selected_indices: ', gt_box3d_selected_indices)

        gt_box3d_tensor = gt_box3d_list[gt_box3d_selected_indices]
        object_id_tensor = torch.from_numpy(np.array(object_id_list)).to(gt_box3d_tensor.device)
        #object_id_tensor = [object_id_list[i] for i in gt_box3d_selected_indices]
        object_id_tensor = object_id_tensor[gt_box3d_selected_indices]
        #print('object_id_tensor: ', object_id_tensor)
        ...
        #print('mask: ', mask)
        #object_id_tensor = [object_id_tensor[i] for i in range(len(mask)) if mask[i]]
        object_id_tensor = object_id_tensor[mask]
        #print('object_id_tensor: ', object_id_tensor)


        return gt_box3d_tensor, object_id_tensor
