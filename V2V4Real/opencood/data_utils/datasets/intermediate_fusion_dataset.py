...
class IntermediateFusionDataset(basedataset.BaseDataset):
    ...
    def __getitem__(self, idx):
        ...
        # MY_CODE
        processed_data_dict['ego'].update({
          'scenario_index': base_data_dict[ego_id]['scenario_index'],
          'timestamp_index': base_data_dict[ego_id]['timestamp_index']
        })
        #print("processed_data_dict['ego']['scenario_index']: ", processed_data_dict['ego']['scenario_index'])
        #print("processed_data_dict['ego']['timestamp_index']: ", processed_data_dict['ego']['timestamp_index'])
    ...
    def collate_batch_train(self, batch):
        ...
        for i in range(len(batch)):
            ...
            scenario_index.append(ego_dict['scenario_index'])
            timestamp_index.append(ego_dict['timestamp_index'])
        ...
        scenario_index = torch.from_numpy(np.array(scenario_index))
        timestamp_index = torch.from_numpy(np.array(timestamp_index))
        ...
        output_dict['ego'].update({'object_bbx_center': object_bbx_center,
                                   ...
                                   'pairwise_t_matrix': pairwise_t_matrix})
        # MY_CODE
        output_dict['ego'].update({'scenario_index': scenario_index,
    ...
    def post_process(self, data_dict, output_dict, return_in_dict=False):
        """
        ...
        """
        gt_box_tensor, gt_object_id_tensor = self.post_processor.generate_gt_bbx(data_dict)

        if return_in_dict:
          pred_box_dict, pred_score_dict, pred_feature_dict, pred_early_feature_dict = \
              self.post_processor.post_process_return_in_dict(data_dict, output_dict)
