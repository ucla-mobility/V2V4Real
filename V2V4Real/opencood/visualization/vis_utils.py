...
def visualize_single_sample_output_gt(pred_tensor,
    ...
    def custom_draw_geometry(pcd, boxes_list):
    ...
    if det_boxes_dict is not None:
      for cav_id in det_boxes_dict.keys():
        oabbs_det = bbx2linset(det_boxes_dict[cav_id], color=color_map[cav_id])
        #print('len(visualize_elements): ', len(visualize_elements))
        visualize_elements = visualize_elements + oabbs_det
        #print('len(visualize_elements): ', len(visualize_elements))
        boxes_list.append(oabbs_det)

    if show_vis:
        #custom_draw_geometry(o3d_pcd, oabbs_pred, oabbs_gt)
        custom_draw_geometry(o3d_pcd, boxes_list)
    if save_path:
