...
from scripts.KITTI import mailpy
...
class trackingEvaluation(object):
    ...
    def __init__(self, t_sha, gt_path="../AB3DMOT/scripts/KITTI", max_truncation = 0, min_height = 25, max_occlusion = 2, \
        mail=None, cls="car", eval_3diou=True, eval_2diou=False, num_hypo=1, thres=None, evaluate_v2v4real=False,
        seq_eval_mode='all', v2v4real_split='val'):
        self.evaluate_v2v4real = evaluate_v2v4real
        self.seq_eval_mode = seq_eval_mode
        self.v2v4real_split = v2v4real_split

        # get number of sequences and
        # get number of frames per sequence from test mapping
        # (created while extracting the benchmark)
        if self.evaluate_v2v4real:
          filename_test_mapping = os.path.join(gt_path, 'v2v4real_%s_evaluate_tracking.seqmap.val' % v2v4real_split)

          #if seq_eval_mode != 'all':
            # assume seq_eval is one of ['0000', '0001', ...]
          #  filename_test_mapping = os.path.join(gt_path, 'v2v4real_%s_seq_%s_evaluate_tracking.seqmap.val' % (v2v4real_split ,seq_eval_mode))
          #else:
          #  filename_test_mapping = os.path.join(gt_path, 'v2v4real_%s_evaluate_tracking.seqmap.val' % v2v4real_split)
        ...
        #print('filename_test_mapping: ', filename_test_mapping)
        ...
        with open(filename_test_mapping, "r") as fh:
            for i,l in enumerate(fh):
                ...
                # v2v4real partial eval
                if self.evaluate_v2v4real:
                  if seq_eval_mode != 'all':
                    if fields[0] != seq_eval_mode:
                      continue
        ...
        self.n_sequences = len(self.n_frames)
        print('self.n_sequences: ', self.n_sequences) # 9
        print('self.n_frames: ', self.n_frames) # [147, 114, 144, 198, 180, 310, 304, 221, 375]
        ...
        # data and parameter
        if self.evaluate_v2v4real:
          self.gt_path           = os.path.join(gt_path, "v2v4real_%s_label" % v2v4real_split)
        else:
        ...
        if self.evaluate_v2v4real:
          self.t_path            = os.path.join("./results/v2v4real", t_sha, "data_%d" % (int(num_hypo)-1))
        else:
...
class stat:
    """
        Utility class to load data.
    """
    def __init__(self, t_sha, cls, suffix, dump, evaluate_v2v4real):
        """
            Constructor, initializes the object given the parameters.
        """
    ...
    def plot(self):
        if self.evaluate_v2v4real:
          save_dir = os.path.join("./results/v2v4real", self.t_sha)
        else:
...
def evaluate(result_sha,mail,num_hypo,eval_3diou,eval_2diou,thres, evaluate_v2v4real=False, seq_eval_mode='all', v2v4real_split='val'):
    """
    ...
    # MY_CODE
    # for v2v4real dataset, only evaluate car class
    if evaluate_v2v4real:
      class_list = ['car']
    else:
      class_list = ["cyclist", "pedestrian", "car"]

    # for c in ("car", "pedestrian", "cyclist"):
    #for c in ("cyclist", "pedestrian", "car"):
    for c in class_list:
        e = trackingEvaluation(t_sha=result_sha, mail=mail,cls=c,eval_3diou=eval_3diou,eval_2diou=eval_2diou,num_hypo=num_hypo,thres=thres, evaluate_v2v4real=evaluate_v2v4real, seq_eval_mode=seq_eval_mode, v2v4real_split=v2v4real_split)
        ...
        stat_meter = stat(t_sha=result_sha, cls=c, suffix=suffix, dump=dump, evaluate_v2v4real=evaluate_v2v4real)
    ...
    evaluation_metrics_dict = {
      'sAMOTA': stat_meter.sAMOTA,
      'AMOTA': stat_meter.amota,
      'AMOTP': stat_meter.amotp
    }
...
if __name__ == "__main__":
    ...
    if len(sys.argv)>=4:
        ...
        if len(sys.argv)>=5: 
          ...
          if len(sys.argv)>=6:
            seq_eval_mode = sys.argv[5] # one of ['0000', '0001', ...]
            if len(sys.argv)>=7:
              v2v4real_split = sys.argv[6] # one of ['train', 'val']
            else:
              v2v4real_split = 'val'
          else:
            seq_eval_mode = 'all'
    ...
    # MY_CODE
    # check if evaluating v2v4real tracking result
    # use detector name inside tracking result folder
    if 'cobevt' in result_sha or 'late_fusion' in result_sha or 'multi_sensor_kalman_filter' in result_sha or 'multi_sensor_differentiable_kalman_filter' in result_sha:
      evaluate_v2v4real = True
    else:
      evaluate_v2v4real = False

    # evaluate results
    success = evaluate(result_sha,mail,num_hypo,eval_3diou,eval_2diou,thres, evaluate_v2v4real, seq_eval_mode, v2v4real_split=v2v4real_split)
